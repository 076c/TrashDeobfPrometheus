...
function visitBlock(block, previsit, postvisit, data, isFunctionBlock)
	table.insert(data.nodeStack, block)
	block.isBlock = true
...
function visitStatement(statement, previsit, postvisit, data)
	table.insert(data.nodeStack, statement)
	...
	elseif
		statement.kind == AstKind.PassSelfFunctionCallStatement or statement.kind == AstKind.FunctionCallStatement
	then
	...
	elseif statement.kind == AstKind.AssignmentStatement then
		for i, primaryExpr in ipairs(statement.lhs) do
			statement.lhs[i] = visitExpression(primaryExpr, previsit, postvisit, data, true)
		end
	...
	if type(postvisit) == "function" then
		...
		if #statements > 0 then
			return table.unpack(statements)
		end
	end
...
function visitExpression(expression, previsit, postvisit, data, assigned)
	table.insert(data.nodeStack, expression)
	expression.isExpression = true
	if type(previsit) == "function" then
		local node, skip = previsit(expression, data, assigned)
	...
	if binaryExpressions[expression.kind] then
		expression.lhs = visitExpression(expression.lhs, previsit, postvisit, data, assigned)
		expression.rhs = visitExpression(expression.rhs, previsit, postvisit, data, assigned)
	end
	...
	then
		expression.base = visitExpression(expression.base, previsit, postvisit, data, assigned)
		for i, arg in ipairs(expression.args) do
			expression.args[i] = visitExpression(arg, previsit, postvisit, data, assigned)
		end
	end
	...
	if expression.kind == AstKind.TableConstructorExpression then
		for i, entry in ipairs(expression.entries) do
			if entry.kind == AstKind.KeyedTableEntry then
				entry.key = visitExpression(entry.key, previsit, postvisit, data, assigned)
			end
			entry.value = visitExpression(entry.value, previsit, postvisit, data, assigned)
		end
	end

	if expression.kind == AstKind.IndexExpression or expression.kind == AstKind.AssignmentIndexing then
		expression.base = visitExpression(expression.base, previsit, postvisit, data, assigned)
		expression.index = visitExpression(expression.index, previsit, postvisit, data, assigned)
	end
	if expression.kind == AstKind.IfElseExpression then
		expression.condition = visitExpression(expression.condition, previsit, postvisit, data, assigned)
		expression.true_expr = visitExpression(expression.true_expr, previsit, postvisit, data, assigned)
		expression.false_expr = visitExpression(expression.false_expr, previsit, postvisit, data, assigned)
	end

	if type(postvisit) == "function" then
		expression = postvisit(expression, data, assigned) or expression
	end
