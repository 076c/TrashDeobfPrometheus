	...
	for i, statement in ipairs(block.statements) do
		if statement.kind ~= AstKind.NopStatement then
			...
			local ws = self:whitespaceIfNeeded2(
				#parts > 0 and parts[#parts] or "",
				self:whitespaceIfNeeded(statementCode, self:newline(true))
			)
			if i ~= 1 then
				parts[#parts + 1] = ws
			end
...
function Unparser:unparseStatement(statement, tabbing)
	...
	elseif statement.kind == AstKind.ForInStatement then
		...
		local bodyCode = self:unparseBlock(statement.body, tabbing)
		push(
			self:whitespaceIfNeeded2(#parts > 0 and parts[#parts] or ""),
			"do",
			self:whitespaceIfNeeded(bodyCode, self:newline(true)),
			bodyCode,
			self:newline(false),
	...
	elseif statement.kind == AstKind.IfStatement then
		...
		for i, eif in ipairs(statement.elseifs) do
			...
			push(
				self:newline(false),
				self:whitespaceIfNeeded2(lastPart, self:tabs(tabbing, true)),
				"elseif",
				self:whitespaceIfNeeded(exprcode),
				exprcode,
				self:whitespaceIfNeeded2(exprcode),
	...
	elseif statement.kind == AstKind.FunctionCallStatement then
		if
			not (statement.base.kind == AstKind.IndexExpression or statement.base.kind == AstKind.VariableExpression)
		then
	...
	elseif self.luaVersion == LuaVersion.LuaU then
		...
		if operator then
			...
			logger:error(
				string.format('"%s" is not a valid unparseable statement in %s!', statement.kind, self.luaVersion)
			)
		end
	end
...
function Unparser:unparseExpression(expression, tabbing)
	...
	if expression.kind == AstKind.GlobalExpression then
	...
	if expression.kind == k or expression.kind == AstKind.AssignmentIndexing then
		...
		if
			...
			or expression.base.kind == AstKind.StringExpression
			or expression.base.kind == AstKind.NumberExpression
			or expression.base.kind == AstKind.NilExpression
		then
			base = "(" .. base .. ")"
		end
	...
	if expression.kind == k then
		if
			not (expression.base.kind == AstKind.IndexExpression or expression.base.kind == AstKind.VariableExpression)
		then
